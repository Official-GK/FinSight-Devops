...
>
> Here in my `kubernetes/risk-engine.yaml` file (point to the code), you can see I have implemented a **Horizontal Pod Autoscaler (HPA)**. Under normal load, we run 3 replicas. However, I have configured the HPA to monitor CPU metrics. If average utilization exceeds 75% due to the market volatility I just triggered, Kubernetes will automatically scale the engine up to 10 pods to absorb the load seamlessly."

## The "Prove It" Live Command (Optional but highly recommended)
Switch to your **Terminal 1** (connected to EC2) and run:
```bash
kubectl get hpa -n finsight
```
> "If we run this command in the live AWS cluster, you can see the HPA actively tracking the CPU targets."
